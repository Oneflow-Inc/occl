...
class Primitives<
  ...
  __device__ __forceinline__ void genericOp(
    ...
    if (tid < nworkers && offset < nelem) {
      ...
      do {
        ...
        if (sharedCollCtx.saveCtx7Quit == 1) {
          ...
          *(blkStatus.barrierCnt + 0 + 0 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
          barrier(); // 我们在这里直接返回，跳过数据搬运，所以把相应的barrier调用了。
          *(blkStatus.barrierCnt + 1 + 0 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
          return;
        }
        ...
        *(blkStatus.barrierCnt + 0 + 1 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
        barrier(); // This barrier has a counterpart in following loop
        *(blkStatus.barrierCnt + 1 + 1 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
    ...
    while (slice < SlicePerChunk && offset < nelem) { // 过滤不了。~~挺巧妙的，用这个条件把nworkers里的线程过滤掉了~~。// +  1023bug: 应该是从这里又让0号等线程钻进了waitPeer，然后设置了SaveCtx7Quit。
      ...
      // }
      *(blkStatus.barrierCnt + 0 + 2 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
      barrier(); // Has couterpart in preceding worker-only loop.
      *(blkStatus.barrierCnt + 1 + 2 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
 ...
 public:
  ...
  __device__ ~Primitives() {
    ...
    if (!(flags & ThreadsSynced)) {
      *(blkStatus.barrierCnt + 0 + 3 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
      barrier();
      *(blkStatus.barrierCnt + 1 + 3 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
    }
    ...
    *(blkStatus.barrierCnt + 0 + 4 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
    barrier();
    *(blkStatus.barrierCnt + 1 + 4 * 2 + tid * NUM_BARRIERS * 2 + blockIdx.x * blockDim.x * NUM_BARRIERS * 2) += 1;
  }
