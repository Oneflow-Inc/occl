...
namespace {
  ...
  run_ring_end:
    if (sharedCollCtx.saveCtx7Quit == 1) {
      ...
      OFCCL_LOG_THRD_0(OFCCL, "Rank<%d> Blk<%d> Thrd<%d>, runRing success, gridOffset = %lu, currentStep = %d, loopSize = %ld", sharedCollCtx.comm.rank, blockIdx.x, tid, gridOffset, currentStep, loopSize);
      __syncthreads(); // ！！！！！！！为了打印log加的！
    }
  }
