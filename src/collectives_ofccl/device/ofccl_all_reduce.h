  ...
  __device__ __forceinline__ void runRing(ncclWorkElem *args) {
    ...
    for (gridOffset = sharedCollCtx[currUsedSlotId].dynamicCollCtx.gridOffset4RunRing; gridOffset < size; gridOffset += loopSize) {
      ...
      if (currentStep < 1) {
        ...
        OFCCL_LOG_RANK_0_THRD_0(OFCCL_MPI2, "Rank<%d> Blk<%d> Thrd<%d> before prims.send. [< 1], gridOffset=%ld, size = %ld, loopSize = %ld currentStep = %d, maxStep = %d", sharedCollCtx[currUsedSlotId].staticCollCtx.rank, blockIdx.x, tid, gridOffset, size, loopSize, currentStep, 2 * nranks - 1);
        // __syncwarp(); // ！！！！！！为了打印log加的！！！！
      ...
      if (currentStep < nranks){
        ...
        OFCCL_LOG_RANK_0_THRD_0(OFCCL_MPI2, "Rank<%d> Blk<%d> Thrd<%d> before prims.directRecvReduceCopySend. [< nranks], gridOffset=%ld, size = %ld, currentStep = %d, maxStep = %d", sharedCollCtx[currUsedSlotId].staticCollCtx.rank, blockIdx.x, tid, gridOffset, size, currentStep, 2 * nranks - 1);
        // __syncwarp(); // ！！！！！！为了打印log加的！！！！
      ...
      if (currentStep < 2 * nranks - 2) { // 2卡不执行这里
        for (int j=currentStep-nranks+1; j<nranks-1; ++j) { // j需要根据currentStep进行相应调整。原来j初值是1. 第一次进入时，currentStep=nranks, j=1
          ...
          OFCCL_LOG_RANK_0_THRD_0(OFCCL_MPI2, "Rank<%d> Blk<%d> Thrd<%d> before prims.directRecvCopySend. [< 2 * nranks - 2], gridOffset=%ld, size = %ld, currentStep = %d, maxStep = %d", sharedCollCtx[currUsedSlotId].staticCollCtx.rank, blockIdx.x, tid, gridOffset, size, currentStep, 2 * nranks - 1);
          // __syncwarp(); // ！！！！！！为了打印log加的！！！！
      ...
      if (currentStep < 2 * nranks - 1) {
        ...
        OFCCL_LOG_RANK_0_THRD_0(OFCCL_MPI2, "Rank<%d> Blk<%d> Thrd<%d> before prims.directRecv. [< 2 * nranks - 1], gridOffset=%ld, size = %ld, currentStep = %d, maxStep = %d", sharedCollCtx[currUsedSlotId].staticCollCtx.rank, blockIdx.x, tid, gridOffset, size, currentStep, 2 * nranks - 1);
        // __syncwarp(); // ！！！！！！为了打印log加的！！！！
