...
static __device__ int sqRead(SQ *sq, SQE *target, int thrdCudaDev) {
  ...
  // OFCCL_LOG(OFCCL, "Rank<%d> Blk<%d> Thrd<%d>, enter, sqReadFrontier = %llu, sq->head=%llu, sq->tail=%llu", thrdCudaDev, blockIdx.x, threadIdx.x, DevRingBufferLogicFrontier(sq, currSqFrontier), DevLogicQHead(sq), DevLogicQTail(sq)); // sharedCollCtx[blkStatus.currLoadedCollId % NUM_SHMEM_SLOT].rank是在loadCtx之后才有效的，在此之前想打印sqRead的情况，需要使用thrdCudaDev，不然会搞出乌龙。
